...
static void
parse_offset_arg(const ContinuousAgg *cagg, Oid offset_type, Datum arg, CaggPolicyOffset *offset)
{
	if (!offset->isnull)
	{
		offset->value = convert_interval_arg(cagg->partition_type, arg, &offset_type, offset->name);
		offset->type = offset_type;
	}
}

static void
parse_cagg_policy_config(const ContinuousAgg *cagg, Oid start_offset_type, Datum start_offset, Oid end_offset_type,
						 Datum end_offset, CaggPolicyConfig *config)
{
...
Datum
policy_refresh_cagg_add_internal(Oid cagg_oid, Oid start_offset_type, Datum start_offset, Oid end_offset_type, Datum end_offset, Interval refresh_interval, bool if_not_exists)
{
	...
	parse_cagg_policy_config(cagg, start_offset_type, start_offset, end_offset_type,end_offset, &policyconf);
...
policy_refresh_cagg_add(PG_FUNCTION_ARGS)
{
	...
	if (PG_ARGISNULL(1) || PG_ARGISNULL(2) || PG_ARGISNULL(3))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("cannot use NULL refresh_start_offset, refresh_end_offset or refresh_schedule_interval")));
	...
	return policy_refresh_cagg_add_internal(cagg_oid, start_offset_type, start_offset, end_offset_type, end_offset,
					 				refresh_interval, if_not_exists);
}
